...
async function detectDuplicates(userId?: string): Promise<void> {
  console.log(`Starting duplicate detection${userId ? ` for user ${userId}` : ' for all users'}`);
  ...
  console.log(`Processing ${bookmarks.length} bookmarks for duplicate detection`);
  ...
  for (const bookmark of bookmarks) {
    ...
    if (userUrlMap.has(normalizedUrl)) {
      ...
      console.log(`Found URL duplicate: ${bookmark.id} (original: ${userUrlMap.get(normalizedUrl)})`);
    ...
    if (bookmark.content_snapshot_path) {
      ...
      if (userContentHashMap.has(contentHash)) {
        ...
        console.log(`Found content duplicate: ${bookmark.id} (original: ${userContentHashMap.get(contentHash)})`);
        await bookmarkRepo.markAsDuplicate(bookmark.id);
        duplicatesFound++;
      } else {
  ...
  console.log(`Duplicate detection complete. Found ${duplicatesFound} duplicates.`);
}
...
async function scanBrokenLinks(userId?: string): Promise<void> {
  console.log(`Starting broken link scan${userId ? ` for user ${userId}` : ' for all users'}`);
  ...
  for (const bookmark of bookmarks) {
    try {
      ...
      const response = await fetch(bookmark.url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; BookmarkManager/1.0; +https://bookmarkmanager.com)',
        },
      });
      ...
      if (response.status >= 400) {
        // 4xx or 5xx error
        console.log(`Broken link detected: ${bookmark.url} (status: ${response.status})`);
      ...
      if (error.name === 'AbortError') {
        ...
        console.log(`Network error for URL: ${bookmark.url} - ${error.message}`);
      }
  ...
  console.log(`Broken link scan complete. Found ${brokenLinksFound} broken links, fixed ${fixedLinks} links.`);
}
...
async function processMaintenanceJob(job: Job<MaintenanceJobData>) {
  ...
  console.log(`Processing maintenance job: ${type}${userId ? ` for user ${userId}` : ''}`);
