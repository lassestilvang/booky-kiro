...
export class BackupWorker {
  ...
  private async processBackupJob(job: Job<BackupJobData>): Promise<void> {
    ...
    console.log(`Processing backup for user ${userId} (auto: ${autoGenerated})`);

    try {
      // Generate backup
      const backup = await this.backupService.generateBackup(userId, autoGenerated);
...
export async function scheduleDailyBackups(
  ...
  const backupQueue = new Queue('backup-queue', { connection: queueConnection });
