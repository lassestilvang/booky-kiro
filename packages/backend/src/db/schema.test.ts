...
import type { BookmarkType, PlanTier } from '@bookmark-manager/shared';
        ...
        fc.asyncProperty(
          ...
          async (bookmarkData) => {
            ...
            try {
              ...
              expect(retrievedBookmark.excerpt).toBe(bookmarkData.excerpt ?? null);
              expect(retrievedBookmark.content_snapshot_path).toBe(
                bookmarkData.contentSnapshotPath ?? null
              );
              expect(retrievedBookmark.content_indexed).toBe(bookmarkData.contentIndexed);
              expect(retrievedBookmark.type).toBe(bookmarkData.type);
              expect(retrievedBookmark.domain).toBe(bookmarkData.domain);
              expect(retrievedBookmark.cover_url).toBe(bookmarkData.coverUrl ?? null);
              expect(retrievedBookmark.is_duplicate).toBe(bookmarkData.isDuplicate);
              expect(retrievedBookmark.is_broken).toBe(bookmarkData.isBroken);
              expect(retrievedBookmark.custom_order).toBe(bookmarkData.customOrder ?? null);
        ...
        fc.asyncProperty(
          ...
          async (collectionData) => {
            ...
            try {
              ...
              expect(retrievedCollection.is_public).toBe(collectionData.isPublic);
              expect(retrievedCollection.share_slug).toBe(uniqueShareSlug);
              expect(retrievedCollection.sort_order).toBe(collectionData.sortOrder);
