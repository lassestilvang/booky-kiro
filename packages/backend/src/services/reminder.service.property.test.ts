...
import { CreateReminderRequest, NotificationChannel } from '@bookmark-manager/shared';
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (remindAt, notificationChannels) => {
          ...
          const reminder = await reminderService.createReminder(testUserId, reminderData);
          ...
          const retrieved = await reminderService.getReminderById(reminder.id, testUserId);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (remindAt) => {
          ...
          const dismissed = await reminderService.dismissReminder(reminder.id, testUserId);
          ...
          const activeReminders = await reminderService.getActiveReminders(testUserId);
          const foundInActive = activeReminders.find(r => r.id === reminder.id);
          expect(foundInActive).toBeUndefined();

          // But it should still appear in all reminders
          const allReminders = await reminderService.getUserReminders(testUserId);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (intervalDays, occurrences) => {
          ...
          const allReminders = await reminderService.getUserReminders(testUserId);
          const ourReminders = allReminders.filter(r => createdReminders.includes(r.id));
          expect(ourReminders.length).toBe(occurrences);

          // Verify reminders are in chronological order
          const sortedReminders = ourReminders.sort((a, b) => 
            a.remindAt.getTime() - b.remindAt.getTime()
          );

          for (let i = 1; i < sortedReminders.length; i++) {
            const timeDiff = sortedReminders[i].remindAt.getTime() - 
          ...
          await reminderService.dismissReminder(createdReminders[0], testUserId);

          // Verify only the first is completed
          const afterDismissal = await reminderService.getUserReminders(testUserId);
          const firstReminder = afterDismissal.find(r => r.id === createdReminders[0]);
          expect(firstReminder!.completed).toBe(true);

          // Verify remaining reminders are still active
          const activeReminders = await reminderService.getActiveReminders(testUserId);
          const activeOurs = activeReminders.filter(r => createdReminders.includes(r.id));
